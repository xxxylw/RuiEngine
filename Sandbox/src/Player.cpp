...
{
	m_ShipTexture = RuiEngine::Texture2D::Create("assets/textures/Ship.png");
}

void Player::OnUpdate(RuiEngine::Timestep ts)
{
	m_Time += ts;

	if (RuiEngine::Input::IsKeyPressed(RE_KEY_SPACE))
	{
...
void Player::OnRender()
{
	...
	RuiEngine::Renderer2D::DrawRotatedQuad({ m_Position.x, m_Position.y, 0.5f }, { 1.0f, 1.3f }, glm::radians(GetRotation()), m_ShipTexture);
}
