...
void Level::Init()
{
	m_TriangleTexture = RuiEngine::Texture2D::Create("assets/textures/Triangle.png");
	m_Player.LoadAssets();
...
void Level::OnUpdate(RuiEngine::Timestep ts)
{
...
void Level::OnRender()
{
	...
	// Background
	RuiEngine::Renderer2D::DrawQuad({ playerPos.x, 0.0f, -0.8f }, { 50.0f, 50.0f }, { 0.3f, 0.3f, 0.3f, 1.0f });

	// Floor and ceiling
	RuiEngine::Renderer2D::DrawQuad({ playerPos.x, 34.0f }, { 50.0f, 50.0f }, color);
	RuiEngine::Renderer2D::DrawQuad({ playerPos.x, -34.0f }, { 50.0f, 50.0f }, color);

	for (auto& pillar : m_Pillars)
	{
		RuiEngine::Renderer2D::DrawRotatedQuad(pillar.TopPosition, pillar.TopScale, glm::radians(180.0f), m_TriangleTexture, 1.0f, color);
		RuiEngine::Renderer2D::DrawRotatedQuad(pillar.BottomPosition, pillar.BottomScale, 0.0f, m_TriangleTexture, 1.0f, color);
	}
