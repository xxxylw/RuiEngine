...
namespace RuiEngine {
	...
	void EditorLayer::OnAttach()
	{
		...
		auto redSquare = m_ActiveScene->CreateEntity("Square Red");
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		if (selectedEntity && m_GizmoType != -1)
		{
			...
			if (ImGuizmo::IsUsing())
			{
				auto [successd, translation, rotation, scale] = Math::DecomposeTransform(transform);
	...
	{
		std::string filepath = FileDialogs::OpenFile("RuiEngine Scene (*.ruiengine)\0*.ruiengine\0");
		if (!filepath.empty())
		{
			m_ActiveScene = CreateRef<Scene>();
			m_ActiveScene->OnViewportResize((uint32_t)m_ViewportSize.x, (uint32_t)m_ViewportSize.y);
